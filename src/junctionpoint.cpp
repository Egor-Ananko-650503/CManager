...
bool CreateJunctionPoint(LPCTSTR origin, LPCTSTR junction)
{
    ...
    nativeTarget.append(QString::fromWCharArray(_T("\\??\\")));
    nativeTarget.append(QString::fromWCharArray(origin));

    // This API only supports Windows-style slashes.
    nativeTarget.replace(_T('/'), _T('\\'));

    // Make sure there's a trailing slash
    if (nativeTarget.right(1)
        != _T("\\")) nativeTarget.append(QString::fromWCharArray(_T(
                                                                     "\\")));

    size_t size = sizeof(REPARSE_DATA_BUFFER) - sizeof(TCHAR)
                  +nativeTarget.length() * sizeof(TCHAR);
    auto_ptr<REPARSE_DATA_BUFFER> reparseBuffer(
        (REPARSE_DATA_BUFFER *)new unsigned char[size]);
    ...
    reparseBuffer->MountPointReparseBuffer.SubstituteNameLength
        = nativeTarget.length() * (int)sizeof(TCHAR);
    ...
    memcpy(reparseBuffer->MountPointReparseBuffer.PathBuffer,
           nativeTarget.toStdWString().c_str(),
           reparseBuffer->MountPointReparseBuffer.SubstituteNameLength);

    // Set ReparseDataLength to the size of the MountPointReparseBuffer
    // Kind in mind that with the padding for the union (given that
    // SymbolicLinkReparseBuffer is larger),
    // this is NOT equal to sizeof(MountPointReparseBuffer)
    reparseBuffer->ReparseDataLength = sizeof(REPARSE_DATA_BUFFER)
    ...
    SafeHandle hDir;
    hDir.Handle = CreateFile(junction,
                             GENERIC_WRITE,
                             0,
                             NULL,
                             OPEN_EXISTING,
    ...
    if (!DeviceIoControl(hDir.Handle, FSCTL_SET_REPARSE_POINT,
                         reparseBuffer.get(), (unsigned int)size, NULL, 0,
                         &bytesReturned, NULL)) {
        RemoveDirectory(junction);
        _tprintf(_T(
                     "Error issuing DeviceIoControl FSCTL_SET_REPARSE_POINT: 0x%x.\r\n"),
                 GetLastError());
        return false;
    }
...
{
    if (attributes == 0) attributes = GetFileAttributes(path);

    if (!IsDirectoryJunction(path, attributes)) return false;

    SafeHandle hDir;
    hDir.Handle = CreateFile(path,
                             GENERIC_READ,
                             0,
                             NULL,
                             OPEN_EXISTING,
    ...
    if (hDir.IsInvalid()) return false;  // Failed to open directory
    ...
    if (!DeviceIoControl(hDir.Handle, FSCTL_GET_REPARSE_POINT, NULL, 0,
                         &ReparseBuffer, MAXIMUM_REPARSE_DATA_BUFFER_SIZE,
                         &dwRet, NULL)) {
    ...
    if (ReparseBuffer.ReparseTag == IO_REPARSE_TAG_MOUNT_POINT) {
        ...
        LPWSTR pPath = ReparseBuffer.MountPointReparseBuffer.PathBuffer
                       +ReparseBuffer.MountPointReparseBuffer.
                       SubstituteNameOffset;
        pPath[ReparseBuffer.MountPointReparseBuffer.SubstituteNameLength] = _T(
            '\0');

        if (wcsncmp(pPath, L"\\??\\", 4) == 0) pPath += 4;  // Skip 'non-parsed'
                                                            // prefix
        ...
        LPWSTR pPath = ReparseBuffer.SymbolicLinkReparseBuffer.PathBuffer
                       +ReparseBuffer.SymbolicLinkReparseBuffer.
                       SubstituteNameOffset;
        pPath[ReparseBuffer.SymbolicLinkReparseBuffer.SubstituteNameLength]
            = _T('\0');

        if (wcsncmp(pPath, L"\\??\\", 4) == 0) pPath += 4;  // Skip 'non-parsed'
                                                            // prefix

        if (ReparseBuffer.SymbolicLinkReparseBuffer.Flags
            &SYMLINK_FLAG_RELATIVE) {
            // It's a relative path. Construct the relative path and resolve
            // destination
            _tcscpy_s(destination, MAX_PATH, path);
            PathCchAddBackslash(destination, sizeof(destination));
...
bool IsDirectoryJunction(LPCTSTR path, DWORD attributes)
{
    ...
    if (attributes == INVALID_FILE_ATTRIBUTES) return false;  // Doesn't exist
    ...
    // Not a directory or not a reparse point
...
bool DeleteJunctionPoint(LPCTSTR path)
{
    ...
    // Does the path exist?
    if (hDir.IsInvalid()) return false;

    DWORD returnedBytes;
    DWORD result = DeviceIoControl(hDir.Handle,
    ...
    if (result == 0) {
        _tprintf_s(_T(
                       "Failed to issue FSCTL_DELETE_REPARSE_POINT. Last error: 0x%x"),
                   GetLastError());
        return false;
    } else {
        return true;
